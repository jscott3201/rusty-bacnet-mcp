...
# or similar in supply-chain work.
#
# Adopted from /Users/justin/Development/selene-db/.github/scripts/check-no-secrets.sh.
...
scan() {
  local pattern="$1"
  local label="$2"
  if git grep -nE "$pattern" -- ':!.github/scripts/' >/dev/null 2>&1; then
    echo "FAIL: matched $label pattern in tracked files:"
    git grep -nE "$pattern" -- ':!.github/scripts/' || true
